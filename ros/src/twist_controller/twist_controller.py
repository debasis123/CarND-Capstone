...
class Controller(object):
    ...
    def control(self, current_vel, dbw_enabled, linear_vel, angular_vel):
        ...
        # if the dbw_enabled is off, reset the Controller, o/w it will accumulate error while it's not self-driving
        if not dbw_enabled:
        ...
        # rospy.logwarn('\nTarget angular velocity: {}'.format(angular_vel))
        # rospy.logwarn('\nTarget velocity: {}'.format(linear_vel))
        # rospy.logwarn('\nCurrent velocity: {}'.format(current_vel))
        # rospy.logwarn('\nFiltered velocity: {}'.format(filtered_vel))
        ...
        if linear_vel == 0 and filtered_vel < 0.1:
            ...
            throttle = 0
            decel = max(vel_error, self.decel_limit)  # negative value
